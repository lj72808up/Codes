...
(function ($) {
  ...
  Topology.prototype = {
    ...
    drawNode: function (v) {
      ...
      var templates = $('#node-templates').clone().removeAttr('id')
        ...
        .click(
          v.clickFunction
          /*function () {
          console.log('====>12312313<=====: '+v.myURL)
          // 2.将tableId放到sessionStorage中, 让fieldMetaData去取
          let info = {
            'tableId': v.tableId
          }
          window.sessionStorage.setItem('metaData_table_id', JSON.stringify(info))
          window.location.href = '#/fieldMetaData'
        }*/)
      if (v.title) {
    ...
    computePathPostion: function (data) {
      ...
      } else if (data.sDirection == 'right' && data.tDirection == 'bottom') {
