<template>
  <div>
    <!--衍生维度创建-->
    <div>
      <h3 style="margin-bottom: 0px; margin-top: 5px;">衍生维度配置</h3>
      <el-row style="margin-top: 10px; width: 100%">
        <el-form ref="dimensionForm" :model="dimensionForm" label-width="110px" :rules="rules">
          ...
          <!--关联主维度, 弹框并显示-->
          <el-form-item label="主维度选择" prop="mainDimension" style="width: 60%">
            <el-input v-model="getMainDimensionName" readonly="readonly" style="width: 400px" size="small"/>
            <el-button size="small" type="primary" plain @click="mainDimensionVisible=true">选择主维度</el-button>
          </el-form-item>

          <el-form-item label="维度描述:" prop="description" style="width: 70%">
            <el-input type="textarea" :autosize="{ minRows: 2, maxRows: 3}" style="width: 100%"
                      v-model="dimensionForm.description"/>
          </el-form-item>

          <el-form-item label="业务标签:" prop="serviceTagIds" style="width: 70%">
      ...
      <el-table :data="factRelationList" stripe style="width: 85%; margin-left:30px" :border="true">
        ...
        <el-table-column key="foreign_key" prop="foreign_key_valid" label="规范符合" width="200" align="center">
          <template slot-scope="scope">
<!--            <span>{{scope.row.foreign_key}}====={{dimensionForm.aliasName}}</span>-->
            <el-button v-if="scope.row.foreign_key===dimensionForm.aliasName" icon="el-icon-s-flag"
    ...
    <el-dialog title="选择主维度" :visible.sync="mainDimensionVisible" >
      <div style="max-height: 500px ; overflow:auto">
        <el-table
          ref="mainDimensionTable"
          :data="mainDimensionList"
          highlight-current-row
          @current-change="onMainDimensionChange"
          style="width: 100%">
          <el-table-column type="index"/>
          <el-table-column property="Name" label="维度名称"/>
          <el-table-column property="AliasName" label="英文名称"/>
  ...
  export default {
    data() {
      return {
        ...
        mainDimensionVisible: false,

      }
    },
    computed: {
      getMainDimensionName: function () {
        let targetId = this.dimensionForm.mainDimensionId
        let targetName = ""
        // console.log("计算触发了 /.... /targetId ;"+ targetId)
        // console.log("计算触发了 /.... / ;"+ JSON.stringify(this.mainDimensionList))
        this.mainDimensionList.forEach(ele => {
          if (ele.Id == targetId) {
    ...
    methods: {
      ...
      onEditDimension(row) {
        this.$nextTick(() => {
          ...
          this.dimensionForm.mainDimensionId = row.mainDimensionId
      ...
      ,
      onMainDimensionChange(val){
        this.$confirm('确认选择<span style="color:red">'+val.Name+'</span>作为主维度吗?', '注意', {
          distinguishCancelAndClose: true,
          dangerouslyUseHTMLString: true,
          confirmButtonText: '确定',
          cancelButtonText: '取消'
        }).then(() => {
          this.mainDimensionVisible = false
          this.dimensionForm.mainDimensionId = val.Id
        })
      },
      onChangeDataSource() {
        let self = this
        ...
        axios.post('/indicator/dictionary/searchById', params, {responseType: 'json'})
          .then(function (response) {
            let row = {   // 接口返回的是json格式, 转换成TableData的形式
              ...
              'mainDimensionId': response.data.MainDimensionId
            }
      ...
      getAllMainDimension(){
        let self = this
        axios.get('/indicator/dimension/getAllMainDimension')
          .then(function (response) {
            self.mainDimensionList = response.data
          }).catch(function (error) {
          console.log(error)
        })
      },
