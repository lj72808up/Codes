...
// @Accept json
// @router /jone/combination [post]
func (this *QueryWsController) QuerySubmit() {
	typeName := this.GetString("typeName", "无线搜索查询")
	...
	// 得到要使用的事实表
	fact, err := sqlModel.DecideFactTb(queryUsingArr)
	if err != nil {
	...
	for _, item := range dimensions {
		if _, ok := keyMapWithFact[item]; ok {
			...
			dimListMq = append(dimListMq, commentMap[dim])
	...
	for _, item := range outputs {
		if _, ok := keyMapWithFact[item]; ok {
			...
			outputListMq = append(outputListMq, commentMap[output])
		}
	}
...
/*	if !models.KylinInfo.CheckKylin(fact.Kylin, dateFilter.Values[0], dateFilter.Values[1]) {
	...
	models.QuerySubmitLog{}.InsertQuerySubmitLog(queryInfo, queryTaskName, queryStr) // string(queryJSONStr)

	// 得到MQ
	queryInfo.Dimensions = dimListMq
	queryInfo.Outputs = outputListMq
...
func LoadQWPartitionAfterTranscoding(fileContent string, userName string) string {
	// 接收文件
	uploadRoot := fmt.Sprintf("%s/%s/%s", models.ServerInfo.UploadRoot, userName,
		utils.DateFormatWithoutLine(time.Now()))
	utils.MkDir(uploadRoot)

	now := time.Now()
	md5Ts := md5.Sum([]byte(fmt.Sprint(now)))
	toFileName := fmt.Sprintf("%s_%s_%x", userName, utils.DateFormatWithoutLine(now), md5Ts)
