...
object TestCEP {
  def main(args: Array[String]): Unit = {
    ...
    val ds: DataStream[Event] = source.map(x => x).keyBy(_.userId)
    ds.print()
     ...
     */
    val pattern: PatternStream[Event] = CEP.pattern(ds, pattern1).inProcessingTime()
    ...
    val result = pattern.process(new PatternProcessFunction[Event, String] {
      override def processMatch(`match`: util.Map[String, util.List[Event]], ctx: PatternProcessFunction.Context, out: Collector[String]): Unit = {
        ...
        out.collect(s"start:{$startEvent} =====> end:{$nextEvent}")
      }
    })
  ...
  case class Event(userId: String, orderId: String, behave: String) {
